...
def _filter_type_to_one_hot(filter_type: str) -> List[float]:
    # Order used in processor: [lowshelf, bell, highshelf, highpass, lowpass]
    mapping = {
        "low-shelf": 0,
        "lowshelf": 0,
        "bell": 1,
...
def _build_batch_preset_from_fined(first_presets: List[Dict], batch_size: int, device: torch.device) -> Dict:
    ...
    for b in range(1, 6):
        eq[f"band_{b}"] = {
            ...
            "gain_db": col(),
            "q": col(),
            "filter_type": torch.zeros(batch_size, 5, device=device, dtype=torch.float32),
        }

    reverb = {
        "wet_gain": col(),
        "room_size": col(),
        "pre_delay": col(),  # ms expected by processor
        "damping": col(),
        "diffusion": col(),
    }
    ...
    for i, p in enumerate(first_presets):
        ...
        for b in range(1, 6):
            ...
            eq[f"band_{b}"]["filter_type"][i] = torch.tensor(
                _filter_type_to_one_hot(band.get("filter_type", "bell")), device=device
            )

        # Reverb: convert pre_delay seconds -> milliseconds for processor
        rv = p.get("Reverb", {})
        reverb["wet_gain"][i, 0] = float(rv.get("wet_gain", 0.5))
        reverb["room_size"][i, 0] = float(rv.get("room_size", 0.5))
        reverb["pre_delay"][i, 0] = float(rv.get("pre_delay", 0.02)) * 1000.0
...
def main():
    ...
    parser.add_argument("--outdir", type=str, required=True, help="Directory to save processed wavs")
